...
<Steps>
  ...
  <Step title="Register the webhook">
    Register your endpoint URL via the API or dashboard and save the signing secret.
  </Step>
...
Register your endpoint via the NAWA API:
...
Copy the `signing_secret` from the response -- it is shown only once.

Available events: `classification.completed`, `classification.failed`, `credits.low`, `credits.exhausted`.
