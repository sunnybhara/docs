---
title: "NAWA API"
sidebarTitle: "Introduction"
description: "Arabic-first AI comment classification. Detect Gulf, Egyptian, Levantine, and MSA dialects at 94%+ accuracy."
---

<img
  className="block dark:hidden"
  src="/images/hero-light.png"
  alt="NAWA API"
  style={{ display: 'none' }}
/>

# The Arabic-first AI classification API

NAWA is purpose-built for Arabic social media. Classify comments by intent, sentiment, dialect, and toxicity — powered by [HUMAIN's ALLaM](https://humain.com) large language model for Arabic.

<CardGroup cols={2}>
  <Card title="Classify Comments" icon="tags" href="/api-reference/classify">
    Detect intent, sentiment, dialect, and toxicity in a single API call. Gulf, Egyptian, Levantine, and MSA supported.
  </Card>
  <Card title="Detect Dialects" icon="language" href="/guides/arabic-dialect-detection">
    Distinguish Gulf from Egyptian, Levantine, and MSA Arabic with 94%+ accuracy using ALLaM.
  </Card>
  <Card title="Generate Replies" icon="reply" href="/api-reference/comments-reply">
    Generate culturally-aware, dialect-matched replies to Arabic comments automatically.
  </Card>
  <Card title="Manage at Scale" icon="chart-mixed" href="/api-reference/analytics">
    Track comment trends, sentiment shifts, and engagement analytics across channels.
  </Card>
</CardGroup>

## Why NAWA?

| Metric | Value |
|--------|-------|
| Dialect accuracy | 94%+ (Gulf, Egyptian, Levantine, MSA) |
| p95 latency | &lt;2 seconds |
| Cost per classify | $0.006 |
| Gross margin | 75% |
| Uptime SLA | 99.9% |

## Quick taste

Make your first classification in seconds:

<CodeGroup>

```bash cURL
curl -X POST https://api.trynawa.com/v1/classify \
  -H "Authorization: Bearer nawa_test_sk_xxx" \
  -H "Content-Type: application/json" \
  -d '{"text": "متى الجزء الثاني؟", "platform": "youtube"}'
```

```typescript TypeScript
import { Nawa } from '@nawalabs/sdk'

const nawa = new Nawa({ apiKey: 'nawa_test_sk_xxx' })
const { data } = await nawa.classify({
  text: 'متى الجزء الثاني؟',
  platform: 'youtube'
})
console.log(data.intent) // "question"
```

```python Python
from nawa import Nawa

nawa = Nawa(api_key="nawa_test_sk_xxx")
result = nawa.classify(
    text="متى الجزء الثاني؟",
    platform="youtube"
)
print(result.data.intent)  # "question"
```

</CodeGroup>

<Card title="Get started in 3 minutes" icon="rocket" href="/quickstart">
  Get your API key and make your first call — no credit card required.
</Card>
