...
<Update label="March 28, 2026" description="v1.1.2" tags={["API", "Provider"]}>
## Jais provider support

Added Jais as a supported AI provider across the NAWA classification pipeline. Jais is now available alongside ALLaM and Claude for routing, audit logging, and client calls.

- **`jais`** -- new value in the `X-NAWA-Provider` response header
- **Provider routing** -- Jais can be selected as a provider in the NAGL pipeline
- **Token costs** -- ALLaM token cost updated to match IBM Watsonx pricing ($0.60/1M tokens)
</Update>
...
- **`X-NAWA-Provider`** -- response header now reflects the active provider (`allam`, `claude`, or `jais`)
</Update>
...
- **Free tier** -- 100 lifetime requests, 14-day key expiry, no credit card required. Same AI models as live.
