...
<Update label="March 23, 2026" description="v1.1.2" tags={["API", "Reliability"]}>
## Claude fallback for reply generation

Automatic Claude fallback added to the reply generation pipeline. If ALLaM returns an empty response during reply generation, NAWA now falls back to Claude instead of returning empty text.

- **Reply generation** -- if ALLaM reply generation fails or returns empty, automatically falls back to Claude
- **ALLaM logging** -- raw ALLaM responses are now logged for debugging empty reply scenarios
- This builds on the v1.1.1 fallback support for `/v1/translate` and `/v1/moderate`
</Update>
...
- **`fallback_used`** -- boolean field in all endpoint responses indicating whether the fallback provider was used
