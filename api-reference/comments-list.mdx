...
| `pagination.total` | integer | Total number of matching results |
...
To paginate through results, increment `offset` by `limit` on each request until `has_more` is `false`.
