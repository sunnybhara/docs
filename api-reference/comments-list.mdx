...
Retrieve comments that have been classified through the API, with filtering and offset-based pagination. This endpoint is **free**.
...
| `comments[].id` | string | Database row ID |
...
<Note>
  Comment history storage is being rolled out. If you see an empty `comments` array with a `message` field, classification storage is not yet enabled for your account. Classifications will be stored and queryable in a future update.
</Note>
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Invalid filter value for `platform`, `intent`, `sentiment`, or `dialect` |
