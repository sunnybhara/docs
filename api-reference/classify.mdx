...
Classify an Arabic or English comment with a single request. Returns intent, sentiment, dialect, and priority analysis powered by NAWA's multi-provider AI pipeline (ALLaM, Claude, Gemini).
...
console.log(result.provider)   // "allam"
```
...
```json
{
  ...
  "result": {
    ...
    "priority": "medium",
    "suggested_reply": {
      "text": "تعليق يسأل عن موعد الجزء الثاني",
      "direction": "rtl"
    },
    "provider": "allam",
...
| `suggested_reply` | object | Suggested reply text and text direction (`ltr` or `rtl`) |
...
<AccordionGroup>
  <Accordion title="Gulf Arabic -- Praise">
    ...
    ```json
    {
      ...
      "result": {
        ...
        "priority": "low",
        "suggested_reply": {
          "text": "تعليق إيجابي يعبر عن إعجاب بالمحتوى",
          "direction": "rtl"
        },
        "provider": "allam",
        "model": "sdaia/allam-1-13b-instruct",
      ...
      "errors": [],
      "request_id": "req_nw_ghi789jkl012abcd"
    }
    ```
  </Accordion>
  ...
  <Accordion title="Egyptian Arabic -- Complaint">
    ...
    ```json
    {
      ...
      "errors": [],
      "request_id": "req_nw_mno345pqr678abcd"
    }
    ```
  </Accordion>
  ...
  <Accordion title="Levantine Arabic -- Suggestion">
    ...
    ```json
    {
      ...
      "errors": [],
      "request_id": "req_nw_stu901vwx234abcd"
    }
    ```
  </Accordion>
