---
title: "Classify comment"
sidebarTitle: "POST /v1/classify"
description: "Classify an Arabic comment by intent, sentiment, dialect, and toxicity in a single API call."
api: "POST https://api.trynawa.com/v1/classify"
---

Classify an Arabic or English comment with a single request. Returns intent, sentiment, dialect, and priority analysis powered by the NAGL routing layer.

## Request

<Note>
  Cost: **$0.006** per request (6 credits). Semantic cache hits are free (`X-NAWA-Cache: HIT`).
</Note>

### Headers

| Header | Required | Description |
|--------|----------|-------------|
| `Authorization` | Yes | `Bearer nawa_live_sk_xxx` or `Bearer nawa_test_sk_xxx` |
| `Content-Type` | Yes | `application/json` |

### Query parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `provider` | string | No | Override AI provider for A/B testing: `claude`, `gemini`, `allam`. |

### Body parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `text` | string | Yes | The comment text to classify. Max 5,000 characters. |
| `platform` | string | No | Source platform: `youtube`, `instagram`, `twitter`, `facebook`. Improves accuracy with platform-specific context. |
| `channel_id` | string | No | Your channel or account identifier for analytics grouping. |
| `metadata` | object | No | Arbitrary key-value metadata to attach to the classification. |

### Example request

<CodeGroup>

```bash cURL
curl -X POST https://api.trynawa.com/v1/classify \
  -H "Authorization: Bearer nawa_test_sk_xxx" \
  -H "Content-Type: application/json" \
  -d '{
    "text": "متى الجزء الثاني؟",
    "platform": "youtube",
    "channel_id": "ch_123"
  }'
```

```typescript TypeScript
import { Nawa } from '@nawalabs/sdk'

const nawa = new Nawa({ apiKey: process.env.NAWA_API_KEY })

const { data, error } = await nawa.classify({
  text: 'متى الجزء الثاني؟',
  platform: 'youtube',
  channelId: 'ch_123'
})
```

```python Python
from nawa import Nawa

nawa = Nawa(api_key="your_api_key")

result = nawa.classify(
    text="متى الجزء الثاني؟",
    platform="youtube",
    channel_id="ch_123"
)
```

</CodeGroup>

## Response

### Success response (200)

```json
{
  "success": true,
  "result": {
    "id": "cls_nw_a1b2c3d4e5f6",
    "object": "classification",
    "intent": "question",
    "sentiment": "neutral",
    "language": "ar",
    "dialect": "gulf",
    "dialect_confidence": 0.95,
    "requires_response": true,
    "priority": "medium",
    "suggested_reply": "إن شاء الله الجزء الثاني قريب!",
    "provider": "allam",
    "model": "nagl-v1",
    "fallback_used": false,
    "tokens_used": 142,
    "cost_usd": 0.006,
    "credits_used": 6,
    "cached": false
  },
  "errors": [],
  "request_id": "req_nw_a1b2c3d4e5f67890"
}
```

### Response headers

| Header | Description |
|--------|-------------|
| `X-Request-Id` | Unique request identifier for debugging |
| `X-RateLimit-Limit` | Rate limit ceiling for current window |
| `X-RateLimit-Remaining` | Requests remaining in current window |
| `X-RateLimit-Reset` | Window reset time (RFC 3339) |
| `X-NAWA-Balance` | Current credit balance in USD |
| `X-NAWA-Balance-Warning` | `low_balance` when below $5 |
| `X-NAWA-Cache` | `HIT` if served from semantic cache (no cost) |
| `X-NAWA-Provider` | AI provider used for this request |
| `X-NAWA-Latency` | Processing latency in milliseconds |
| `X-NAWA-Calibration-Version` | Calibration version applied (Enterprise only) |

### Result fields

| Field | Type | Description |
|-------|------|-------------|
| `id` | string | Unique classification ID (prefix `cls_`) |
| `object` | string | Always `"classification"` |
| `intent` | string | `question`, `complaint`, `praise`, `suggestion`, `spam`, `other` |
| `sentiment` | string | `positive`, `negative`, `neutral`, `mixed` |
| `language` | string | Detected language code (`ar`, `en`) |
| `dialect` | string | `gulf`, `egyptian`, `levantine`, `msa` |
| `dialect_confidence` | number | Confidence score (0-1) |
| `requires_response` | boolean | Whether the comment warrants a reply |
| `priority` | string | Priority level: `high`, `medium`, `low` |
| `suggested_reply` | string or null | AI-generated reply suggestion |
| `provider` | string | AI provider used (`allam`, `claude`, `gemini`) |
| `model` | string | Model version used for classification |
| `fallback_used` | boolean | Whether a fallback provider was used |
| `tokens_used` | number | Token count consumed by the AI model |
| `cost_usd` | number | Cost in USD |
| `credits_used` | number | Credits deducted |
| `cached` | boolean | Whether this was served from semantic cache |

### Error responses

| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text`, invalid `platform`, text too long |
| 401 | `authentication_error` | Invalid or missing API key |
| 402 | `insufficient_credits` | No credits remaining |
| 429 | `rate_limit_error` | Rate limit exceeded |
| 500 | `api_error` | Internal or provider error |

### More Arabic examples

<AccordionGroup>
  <Accordion title="Gulf Arabic -- Praise">
    ```bash
    curl -X POST https://api.trynawa.com/v1/classify \
      -H "Authorization: Bearer nawa_test_sk_xxx" \
      -H "Content-Type: application/json" \
      -d '{"text": "ما شاء الله عليك، محتوى رهيب!", "platform": "youtube"}'
    ```

    Response:
    ```json
    {
      "success": true,
      "result": {
        "id": "cls_nw_ghi789jkl012",
        "object": "classification",
        "intent": "praise",
        "sentiment": "positive",
        "language": "ar",
        "dialect": "gulf",
        "dialect_confidence": 0.93,
        "requires_response": false,
        "priority": "low",
        "suggested_reply": null,
        "provider": "allam",
        "model": "nagl-v1",
        "fallback_used": false,
        "tokens_used": 128,
        "cost_usd": 0.006,
        "credits_used": 6,
        "cached": false
      },
      "errors": [],
      "request_id": "req_nw_ghi789jkl012"
    }
    ```
  </Accordion>

  <Accordion title="Egyptian Arabic -- Complaint">
    ```bash
    curl -X POST https://api.trynawa.com/v1/classify \
      -H "Authorization: Bearer nawa_test_sk_xxx" \
      -H "Content-Type: application/json" \
      -d '{"text": "الصوت وحش أوي في الفيديو ده", "platform": "youtube"}'
    ```

    Response:
    ```json
    {
      "success": true,
      "result": {
        "id": "cls_nw_mno345pqr678",
        "object": "classification",
        "intent": "complaint",
        "sentiment": "negative",
        "language": "ar",
        "dialect": "egyptian",
        "dialect_confidence": 0.97,
        "requires_response": true,
        "priority": "high",
        "suggested_reply": "معلش، هنشتغل على الصوت في الفيديوهات الجاية!",
        "provider": "allam",
        "model": "nagl-v1",
        "fallback_used": false,
        "tokens_used": 135,
        "cost_usd": 0.006,
        "credits_used": 6,
        "cached": false
      },
      "errors": [],
      "request_id": "req_nw_mno345pqr678"
    }
    ```
  </Accordion>

  <Accordion title="Levantine Arabic -- Suggestion">
    ```bash
    curl -X POST https://api.trynawa.com/v1/classify \
      -H "Authorization: Bearer nawa_test_sk_xxx" \
      -H "Content-Type: application/json" \
      -d '{"text": "لو سمحت حاول تحكي عن المطاعم بلبنان", "platform": "instagram"}'
    ```

    Response:
    ```json
    {
      "success": true,
      "result": {
        "id": "cls_nw_stu901vwx234",
        "object": "classification",
        "intent": "suggestion",
        "sentiment": "neutral",
        "language": "ar",
        "dialect": "levantine",
        "dialect_confidence": 0.96,
        "requires_response": true,
        "priority": "medium",
        "suggested_reply": null,
        "provider": "allam",
        "model": "nagl-v1",
        "fallback_used": false,
        "tokens_used": 140,
        "cost_usd": 0.006,
        "credits_used": 6,
        "cached": false
      },
      "errors": [],
      "request_id": "req_nw_stu901vwx234"
    }
    ```
  </Accordion>

  <Accordion title="Provider override (A/B testing)">
    Force classification through a specific AI provider using the `provider` query parameter:

    ```bash
    curl -X POST "https://api.trynawa.com/v1/classify?provider=claude" \
      -H "Authorization: Bearer nawa_test_sk_xxx" \
      -H "Content-Type: application/json" \
      -d '{"text": "متى الجزء الثاني؟"}'
    ```

    The `X-NAWA-Provider` response header confirms which provider processed the request.
  </Accordion>
</AccordionGroup>
