...
Classify text with a single request. Returns intent, sentiment, dialect, priority, and a suggested reply direction powered by HUMAIN's ALLaM model.
...
| `context` | object | No | Optional context for classification (e.g. platform, channel metadata). |
...
```json
{
  ...
  "result": {
    ...
    "priority": "medium",
    "suggested_reply": {
      "text": "سؤال عن موعد الجزء التالي",
      "direction": "rtl"
    },
    "provider": "allam",
...
| `dialect` | string | Arabic dialect: `gulf`, `egyptian`, `levantine`, `msa`. Null if not Arabic. |
...
| `suggested_reply` | object | Suggested reply with `text` and `direction` (ltr/rtl). Null if no reply needed. |
...
| `tokens_used` | integer | Token count (null if not tracked) |
