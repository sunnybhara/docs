...
Classify Arabic or English text with a single request. Returns intent, sentiment, dialect, priority, and a suggested reply direction powered by the NAGL pipeline (ALLaM, Claude, or Gemini).
...
|-----------|------|----------|-------------|
| `provider` | string | No | Force a specific AI provider for A/B testing: `claude`, `gemini`, `allam`. Bypasses language-based routing. |
...
Pass `?provider=` to bypass NAWA's automatic language-based routing:
...
```json
{
  ...
  "result": {
    ...
    "priority": "medium",
    "suggested_reply": {
      "text": "سؤال من المتابع عن الجزء القادم من المحتوى",
      "direction": "rtl"
    },
    "provider": "allam",
...
| `requires_response` | boolean | Whether the comment requires a reply |
| `priority` | string | `high`, `medium`, or `low` |
| `suggested_reply` | object | AI reasoning or suggested reply direction with `text` and `direction` (`ltr`/`rtl`) |
...
<AccordionGroup>
  ...
  <Accordion title="English text (auto-routed to Claude)">
    ```bash
    curl -X POST https://api.trynawa.com/v1/classify \
      -H "Authorization: Bearer nawa_test_sk_xxx" \
      -H "Content-Type: application/json" \
