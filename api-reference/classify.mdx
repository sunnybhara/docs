...
Classify Arabic or English text with a single request. Returns intent, sentiment, dialect, priority, and a suggested reply direction. Supports semantic caching and multi-provider routing through the NAGL pipeline.
...
| `suggested_reply` | object | Contains `text` (summary/reply hint) and `direction` (`ltr` or `rtl`) |
| `provider` | string | AI provider used: `claude`, `gemini`, or `allam` |
| `model` | string | Model identifier |
| `fallback_used` | boolean | Whether a fallback provider was used |
| `tokens_used` | integer \| null | Token count (null when not tracked) |
...
| 402 | `permission_error` | No credits remaining (live keys only) |
