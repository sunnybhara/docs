...
Classify Arabic or English text with a single request. Returns intent, sentiment, dialect, priority, and a suggested reply direction powered by HUMAIN ALLaM.
...
| `requires_response` | boolean | Whether the comment expects a response |
...
<AccordionGroup>
  <Accordion title="Gulf Arabic -- question">
    ...
    ```json
    {
      ...
      "result": {
        ...
        "priority": "high",
        "suggested_reply": {
          "text": "سؤال عن موعد إصدار جزء جديد",
          "direction": "rtl"
        },
        "provider": "allam",
        "model": "sdaia/allam-1-13b-instruct",
  ...
  <Accordion title="English -- with provider override">
    ```bash
