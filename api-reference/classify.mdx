---
title: "Classify Comment"
sidebarTitle: "POST /v1/classify"
description: "Classify Arabic or English text by intent, sentiment, dialect, and priority in a single API call."
api: "POST https://api.trynawa.com/v1/classify"
---

Classify Arabic or English text with a single request. Returns intent, sentiment, dialect, priority, and a suggested reply powered by HUMAIN's ALLaM model with NAGL routing.

## Request

<Note>
  Cost: **$0.006** per request (6 credits). Semantic cache hits are free (`X-NAWA-Cache: HIT`).
</Note>

### Headers

| Header | Required | Description |
|--------|----------|-------------|
| `Authorization` | Yes | `Bearer nawa_live_sk_xxx` or `Bearer nawa_test_sk_xxx` |
| `Content-Type` | Yes | `application/json` |

### Query parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `provider` | string | No | Force a specific AI provider for A/B testing: `claude`, `gemini`, `allam`. Bypasses automatic language-based routing. |

### Body parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `text` | string | Yes | The text to classify. Must be non-empty. |
| `context` | object | No | Optional context object to provide additional classification context. |

### Example request

<CodeGroup>

```bash cURL
curl -X POST https://api.trynawa.com/v1/classify \
  -H "Authorization: Bearer nawa_test_sk_xxx" \
  -H "Content-Type: application/json" \
  -d '{
    "text": "متى الجزء الثاني؟"
  }'
```

```typescript TypeScript
import { Nawa } from '@nawalabs/sdk'

const nawa = new Nawa({ apiKey: process.env.NAWA_API_KEY })

const { data, error } = await nawa.classify({
  text: 'متى الجزء الثاني؟'
})
```

```python Python
from nawa import Nawa

nawa = Nawa(api_key="your_api_key")

result = nawa.classify(
    text="متى الجزء الثاني؟"
)
```

</CodeGroup>

## Response

### Success response (200)

```json
{
  "success": true,
  "result": {
    "id": "cls_nw_pon1yom9bkfe",
    "object": "classification",
    "intent": "question",
    "sentiment": "neutral",
    "language": "ar",
    "dialect": "gulf",
    "dialect_confidence": 0.95,
    "requires_response": true,
    "priority": "medium",
    "suggested_reply": {
      "text": "The commenter is asking about a follow-up or sequel.",
      "direction": "rtl"
    },
    "provider": "allam",
    "model": "sdaia/allam-1-13b-instruct",
    "fallback_used": false,
    "tokens_used": null,
    "cost_usd": 0.006,
    "credits_used": 6
  },
  "errors": [],
  "request_id": "req_abc123def456"
}
```

### Response headers

| Header | Description |
|--------|-------------|
| `X-Request-Id` | Unique request identifier for debugging |
| `X-RateLimit-Limit` | Rate limit ceiling for current window |
| `X-RateLimit-Remaining` | Requests remaining in current window |
| `X-RateLimit-Reset` | Window reset time (RFC 3339) |
| `X-NAWA-Balance` | Current credit balance in USD |
| `X-NAWA-Balance-Warning` | `low_balance` when below $5 |
| `X-NAWA-Cache` | `HIT` if served from semantic cache (no cost), `MISS` otherwise |
| `X-NAWA-Provider` | AI provider used for this request (`allam`, `claude`, `gemini`) |
| `X-NAWA-Latency` | Server-side processing time in milliseconds |
| `X-NAWA-Calibration-Version` | Calibration profile version applied (Enterprise only) |

### Result fields

| Field | Type | Description |
|-------|------|-------------|
| `id` | string | Classification ID (`cls_nw_` prefix) |
| `object` | string | Always `classification` |
| `intent` | string | Detected intent: `question`, `complaint`, `praise`, `suggestion`, `spam`, `other` |
| `sentiment` | string | `positive`, `negative`, `neutral`, `mixed` |
| `language` | string | Detected language code: `ar`, `en`, `mixed` |
| `dialect` | string or null | Detected Arabic dialect: `gulf`, `egyptian`, `levantine`, `msa`. Null if not Arabic. |
| `dialect_confidence` | number or null | Confidence score (0-1) for dialect detection. Null if not Arabic. |
| `requires_response` | boolean | Whether the comment warrants a reply |
| `priority` | string | Suggested priority: `high`, `medium`, `low` |
| `suggested_reply` | object | Contains `text` (reasoning or suggested reply) and `direction` (`rtl` or `ltr`) |
| `provider` | string | AI provider used: `allam`, `claude`, `gemini` |
| `model` | string | Specific model identifier used |
| `fallback_used` | boolean | Whether a fallback provider was used |
| `tokens_used` | integer or null | Token count (null when not tracked) |
| `cost_usd` | number | Cost in USD for this request |
| `credits_used` | integer | Credits deducted (6 per request, 0 for cache hits) |

### Error responses

| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text`, invalid `provider` value, malformed JSON |
| 401 | `authentication_error` | Invalid or missing API key |
| 402 | `insufficient_credits` | No credits remaining |
| 429 | `rate_limit_error` | Rate limit exceeded |
| 500 | `api_error` | Internal or provider error |

### More Arabic examples

<AccordionGroup>
  <Accordion title="Gulf Arabic -- Praise">
    ```bash
    curl -X POST https://api.trynawa.com/v1/classify \
      -H "Authorization: Bearer nawa_test_sk_xxx" \
      -H "Content-Type: application/json" \
      -d '{"text": "ما شاء الله عليك، محتوى رهيب!"}'
    ```

    Response:
    ```json
    {
      "success": true,
      "result": {
        "id": "cls_nw_ghi789jkl012",
        "object": "classification",
        "intent": "praise",
        "sentiment": "positive",
        "language": "ar",
        "dialect": "gulf",
        "dialect_confidence": 0.93,
        "requires_response": false,
        "priority": "low",
        "suggested_reply": {
          "text": "The commenter is expressing admiration for the content.",
          "direction": "rtl"
        },
        "provider": "allam",
        "model": "sdaia/allam-1-13b-instruct",
        "fallback_used": false,
        "tokens_used": null,
        "cost_usd": 0.006,
        "credits_used": 6
      },
      "errors": [],
      "request_id": "req_ghi789jkl012"
    }
    ```
  </Accordion>

  <Accordion title="Egyptian Arabic -- Complaint">
    ```bash
    curl -X POST https://api.trynawa.com/v1/classify \
      -H "Authorization: Bearer nawa_test_sk_xxx" \
      -H "Content-Type: application/json" \
      -d '{"text": "الصوت وحش أوي في الفيديو ده"}'
    ```

    Response:
    ```json
    {
      "success": true,
      "result": {
        "id": "cls_nw_mno345pqr678",
        "object": "classification",
        "intent": "complaint",
        "sentiment": "negative",
        "language": "ar",
        "dialect": "gulf",
        "dialect_confidence": 0.92,
        "requires_response": true,
        "priority": "high",
        "suggested_reply": {
          "text": "The commenter is reporting a technical audio issue.",
          "direction": "rtl"
        },
        "provider": "allam",
        "model": "sdaia/allam-1-13b-instruct",
        "fallback_used": false,
        "tokens_used": null,
        "cost_usd": 0.006,
        "credits_used": 6
      },
      "errors": [],
      "request_id": "req_mno345pqr678"
    }
    ```
  </Accordion>

  <Accordion title="Levantine Arabic -- Suggestion">
    ```bash
    curl -X POST https://api.trynawa.com/v1/classify \
      -H "Authorization: Bearer nawa_test_sk_xxx" \
      -H "Content-Type: application/json" \
      -d '{"text": "لو سمحت حاول تحكي عن المطاعم بلبنان"}'
    ```

    Response:
    ```json
    {
      "success": true,
      "result": {
        "id": "cls_nw_stu901vwx234",
        "object": "classification",
        "intent": "suggestion",
        "sentiment": "neutral",
        "language": "ar",
        "dialect": "gulf",
        "dialect_confidence": 0.88,
        "requires_response": true,
        "priority": "medium",
        "suggested_reply": {
          "text": "The commenter is requesting specific content about restaurants in Lebanon.",
          "direction": "rtl"
        },
        "provider": "allam",
        "model": "sdaia/allam-1-13b-instruct",
        "fallback_used": false,
        "tokens_used": null,
        "cost_usd": 0.006,
        "credits_used": 6
      },
      "errors": [],
      "request_id": "req_stu901vwx234"
    }
    ```
  </Accordion>

  <Accordion title="Provider override for A/B testing">
    Force classification through Claude instead of the default provider:
    ```bash
    curl -X POST "https://api.trynawa.com/v1/classify?provider=claude" \
      -H "Authorization: Bearer nawa_test_sk_xxx" \
      -H "Content-Type: application/json" \
      -d '{"text": "متى الجزء الثاني؟"}'
    ```
  </Accordion>
</AccordionGroup>
