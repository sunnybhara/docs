...
| `detected_script` | string or null | Script detected by ADT: `arabic`, `latin`, or `mixed`. Present when the input contains Latin-script Arabic (Arabizi). |
| `transliterated_text` | string or null | Arabic-script transliteration of the original Arabizi text. `null` if the input was already in Arabic script. |
| `transliteration_confidence` | number or null | Confidence score (0-1) for the Arabizi-to-Arabic transliteration. |
| `adt_version` | string or null | Version of the ADT (Arabizi Detection and Transliteration) module that performed the transliteration. |
| `arabizi_original_text` | string or null | The original Latin-script Arabizi text before transliteration. `null` if no transliteration occurred. |

<Info>
  ADT fields (`detected_script`, `transliterated_text`, `transliteration_confidence`, `adt_version`, `arabizi_original_text`) are only present when the input text contains Latin-script Arabic (Arabizi). For standard Arabic-script or English input, these fields are omitted.
</Info>
