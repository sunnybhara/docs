...
Classify Arabic or English text with a single request. Returns intent, sentiment, dialect, priority, and a suggested reply direction. Powered by the NAGL pipeline with ALLaM, Claude, or Gemini providers.
...
```json
{
  ...
  "result": {
    ...
    "priority": "medium",
    "suggested_reply": {
      "text": "سؤال عن الجزء القادم من المحتوى",
      "direction": "rtl"
    },
    "provider": "allam",
...
| `suggested_reply` | object | Contains `text` (summary/reasoning) and `direction` (`ltr` or `rtl`) |
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text`, invalid `provider`, empty text |
  ...
  <Accordion title="Provider override (Claude)">
    ...
    ```json
    {
      ...
      "result": {
        ...
        "priority": "low",
        "suggested_reply": {
          "text": "اقتراح لتغطية مطاعم في لبنان",
          "direction": "rtl"
        },
        "provider": "claude",
