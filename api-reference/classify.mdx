...
Classify an Arabic or English comment with a single request. Returns intent, sentiment, dialect, priority, and a suggested reply powered by HUMAIN's ALLaM model.
...
| `requires_response` | boolean | Whether this comment likely needs a reply |
| `priority` | string | Suggested response priority: `low`, `medium`, `high` |
| `suggested_reply` | object | Suggested reply with `text` and `direction` |
  ...
  <Accordion title="Egyptian Arabic - Complaint">
    ...
    ```json
    {
      ...
      "result": {
        ...
        "priority": "high",
        "suggested_reply": {
          "text": "معلش، هنحسن الصوت في الفيديوهات الجاية إن شاء الله",
          "direction": "rtl"
        },
        "provider": "allam",
        "model": "nagl-v1",
