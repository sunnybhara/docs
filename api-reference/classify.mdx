...
| `suggested_reply` | object | Contains `text` (reasoning/summary) and `direction` (`ltr` or `rtl`) |
...
<AccordionGroup>
  ...
  <Accordion title="Levantine Arabic -- Suggestion">
    ...
    ```json
    {
      ...
      "result": {
        ...
        "priority": "low",
        "suggested_reply": {
          "text": "اقتراح للتحدث عن المطاعم في لبنان",
          "direction": "rtl"
        },
        "provider": "allam",
        "model": "sdaia/allam-1-13b-instruct",
  ...
  <Accordion title="Using provider override for A/B testing">
    ```bash
