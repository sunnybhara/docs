...
<Note>
  The health endpoint returns a raw JSON response (not wrapped in the standard `success`/`result` envelope). Returns HTTP 200 when healthy, HTTP 503 when degraded.
</Note>
