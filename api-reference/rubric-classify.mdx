...
Classify a comment against a custom rubric. Define your own categories and scoring criteria for domain-specific classification. Supports both single-label and multi-label classification.
...
| `rubric.categories` | object[] | Yes | Array of category objects. Max 20 categories. Each must have a `name` and optionally a `description`. |
| `rubric.multi_label` | boolean | No | Allow multiple categories. Default: `false`. |
...
```json
{
  ...
  "errors": [],
  "request_id": "req_nw_rub123abc456789"
}
```
...
| `categories` | object[] | Array of categories with `name` and `confidence` score (0-1), sorted by confidence descending |
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text` or `rubric`. No categories. More than 20 categories. |
