...
Classify text against a custom rubric. Define your own categories and scoring criteria for domain-specific classification. Supports multi-label classification and configurable confidence thresholds.

<Note>
  Cost: **3 credits** ($0.003) per request. **500 free requests/month** on this endpoint, no credit card required.
</Note>
...
| `rubric.categories` | array | Yes | Array of category objects, each with a `name` (string, required) and optional `description` (string). Max 20 categories. |
...
```json
{
  ...
  "errors": [],
  "request_id": "req_nw_rub123abc456defghi78"
}
```
...
| 402 | `insufficient_credits` | No credits remaining (does not apply to free tier quota) |
