...
Classify text against a user-defined rubric. Define your own categories and let NAWA score each one. Supports multi-label classification and configurable confidence thresholds.
...
| `rubric.categories` | array | Yes | Array of category objects. Each must have a `name` (string) and optionally a `description` (string). Maximum 20 categories. |
| `rubric.multi_label` | boolean | No | If `true`, returns all categories above the confidence threshold. Default: `false`. |
...
```json
{
  "result": {
    ...
    "categories": [
      ...
      { "name": "technical_issue", "confidence": 0.12 }
    ],
    "multi_label": false,
    "language": "ar",
    "provider": "allam",
...
| `categories` | array | Matched categories with `name` and `confidence` (0-1) |
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text` or `rubric`, invalid categories, exceeds 20 category limit |
