...
Classify a comment against a custom rubric. Define your own categories with optional descriptions for domain-specific classification. Supports multi-label mode and configurable confidence thresholds.
...
| `rubric.categories` | object[] | Yes | Array of category objects. Each must have a `name` (string) and optional `description` (string). 1-20 categories. |
...
| `categories` | object[] | Matched categories with `name` and `confidence` score, filtered by threshold |
...
<Tip>
  When `multi_label` is `false`, the `categories` array contains all categories sorted by confidence. When `true`, only categories above the `confidence_threshold` are returned.
</Tip>
