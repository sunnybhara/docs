...
Classify text against a custom rubric. Define your own categories, optional descriptions, and confidence thresholds for domain-specific classification. Supports multi-label classification.
...
| `rubric.categories` | array | Yes | Array of category objects. Each must have a `name` (string) and optional `description` (string). Min 1, max 20 categories. |
...
```json
{
  ...
  "errors": [],
  "request_id": "req_nw_rub123abc456defghijk78"
}
```
...
| `categories` | array | Matched categories with `name` and `confidence` (0-1). Filtered by `confidence_threshold`. |
