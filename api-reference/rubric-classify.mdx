...
| `rubric.categories` | object[] | Yes | Array of category objects. Max 20 categories. |
| `rubric.categories[].name` | string | Yes | Category name. |
| `rubric.categories[].description` | string | No | Description to guide the model on this category. |
...
When `multi_label` is `true`, multiple categories can match:
...
| `categories` | object[] | Matched categories, each with `name` and `confidence` (0-1) |
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text` or `rubric.categories`, invalid category format, exceeds 20 categories |
