...
Classify text against a custom rubric. Define your own categories (up to 20) for domain-specific classification. Supports single-label and multi-label modes with configurable confidence thresholds.
...
| `rubric.categories` | array | Yes | Array of category objects. Each object has a `name` (string, required) and optional `description` (string). Max 20 categories. |
| `rubric.multi_label` | boolean | No | Allow multiple categories to match. Default: `false` (single best match). |
...
|-------|------|-------------|
| `id` | string | Rubric classification ID (`rcl_nw_` prefix) |
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text` or `rubric`. Invalid rubric structure. More than 20 categories. |
