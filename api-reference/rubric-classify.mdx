...
Classify a comment against a custom rubric. Define your own categories with descriptions for domain-specific classification. Supports multi-label classification with configurable confidence thresholds.
...
| `rubric.categories` | object[] | Yes | Array of category objects, each with `name` (required) and `description` (optional). |
...
| `multi_label` | boolean | Whether multiple categories were matched |
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text` or `rubric`, empty categories, text too long |
