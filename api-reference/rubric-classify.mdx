...
Classify text against a custom rubric. Define your own categories with optional descriptions for domain-specific classification.

<Note>
  Cost: **$0.003** per request (3 credits). This endpoint does not use caching.
</Note>
...
| `rubric.categories` | object[] | Yes | Array of category objects (1-20). Each must have a `name` and optional `description`. |
| `rubric.multi_label` | boolean | No | Return all matching categories above threshold. Default: `false` (single best match). |
