...
| `rubric.categories` | array | Yes | Array of category objects. Each must have a `name` (string, required) and optional `description` (string). Max 20 categories. |
...
| `categories` | array | Matched categories with `name` (string) and `confidence` (number, 0-1) |
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text` or `rubric`. Invalid categories (max 20). Empty category names. |
