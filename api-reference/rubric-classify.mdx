...
Classify text against a custom rubric. Define your own categories, enable multi-label classification, and set confidence thresholds.
...
| `rubric.categories` | object[] | Yes | Array of category objects (1-20 items). Each must have a `name` (string). Optionally include a `description` (string) to guide the model. |
...
console.log(data.result.categories) // [{ name: "translation_request", confidence: 0.94 }]
```
...
data = response.json()
print(data["result"]["categories"])  # [{"name": "translation_request", "confidence": 0.94}]
```
...
| `categories` | object[] | Matched categories with `name` and `confidence` (0-1). Filtered by your `confidence_threshold`. |
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text`, `rubric`, or `rubric.categories`; more than 20 categories; empty category name; invalid `provider` |
