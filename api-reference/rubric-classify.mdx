...
description: "Classify a comment against a custom rubric with user-defined categories and confidence thresholds."
...
<Tip>
  Categories below the `confidence_threshold` are filtered out of the response. If no category meets the threshold, the `categories` array is empty.
</Tip>

### Multi-label example

Set `multi_label: true` to allow a comment to match multiple categories:
...
curl -X POST "https://api.trynawa.com/v1/rubric-classify" \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "text": "الصوت مو واضح وبعد وين الترجمة؟",
    "rubric": {
      "categories": [
...
```json
{
  ...
  "result": {
    ...
    "categories": [
      ...
      {"name": "technical_issue", "confidence": 0.76}
    ],
    "multi_label": true,
    "language": "ar",
    "provider": "allam",
  ...
  "errors": [],
  "request_id": "req_nw_rcl2f3g4h5i6j"
}
```
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text` or `rubric`. Empty categories array. More than 20 categories. Category missing `name`. |
