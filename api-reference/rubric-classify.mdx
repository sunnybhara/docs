...
<Note>
  Cost: **$0.003** per request (3 credits). Available on the free tier.
</Note>
...
| `rubric.categories` | object[] | Yes | Array of category objects with `name` and optional `description`. Max 20 categories. |
...
| `categories` | object[] | Array of `{name, confidence}` objects for all rubric categories |
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text` or `rubric`, too many categories (max 20) |
