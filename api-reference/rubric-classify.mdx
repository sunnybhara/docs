...
Classify text against a user-defined rubric. Define your own categories with descriptions for domain-specific classification. Supports multi-label mode and configurable confidence thresholds.
...
| `rubric.categories` | object[] | Yes | Array of category objects, each with `name` (required) and `description` (optional). Max 20 categories. |
...
print(result.categories[0].name)        # 'translation_request'
print(result.categories[0].confidence)  # 0.94
```
...
console.log(data.categories[0].name)        // 'translation_request'
console.log(data.categories[0].confidence)  // 0.94
```
