...
Classify text against a custom rubric. Define your own categories with optional descriptions for domain-specific classification. Supports multi-label mode and configurable confidence thresholds.

<Note>
  Cost: **$0.003** per request (3 credits). Maximum 20 categories per rubric.
</Note>
...
| `rubric.categories` | object[] | Yes | Array of category objects. Each must have a `name` (string). Optional `description` (string) to guide the model. 1--20 categories. |
...
| `categories` | object[] | Matching categories, each with `name` (string) and `confidence` (0--1) |
...
| `credits_used` | integer | Credits deducted (3 for rubric classify) |
