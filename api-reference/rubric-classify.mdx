...
description: "Classify a comment against a custom rubric with multi-label support and configurable thresholds."
...
| `rubric.confidence_threshold` | number | No | Minimum confidence score (0-1) for a category to be returned. Default: `0.5`. |
...
```json
{
  ...
  "result": {
    ...
    "categories": [
      { "name": "translation_request", "confidence": 0.94 },
      { "name": "technical_issue", "confidence": 0.62 }
    ],
    "multi_label": true,
    "language": "ar",
    "provider": "allam",
...
| `categories` | object[] | Matching categories with `name` and `confidence` (0-1) |
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text` or `rubric`, empty categories, invalid threshold |
