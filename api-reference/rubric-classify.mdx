...
Classify text against a custom rubric. Define your own categories, descriptions, and scoring criteria for domain-specific classification. Supports multi-label classification and configurable confidence thresholds.
...
|-----------|------|----------|-------------|
| `text` | string | Yes | The text to classify. Must not be empty. |
| `rubric` | object | Yes | The rubric definition. |
| `rubric.categories` | array | Yes | Array of category objects. Min 1, max 20. |
| `rubric.categories[].name` | string | Yes | Category name. |
| `rubric.categories[].description` | string | No | Description to guide the model. |
| `rubric.multi_label` | boolean | No | Return all matching categories above threshold. Default: `false`. |
...
```json
{
  ...
  "result": {
    ...
    "categories": [
      ...
      {"name": "content_feedback", "confidence": 0.12},
...
|-------|------|-------------|
| `id` | string | Unique classification ID (format: `rcl_nw_xxx`) |
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text` or `rubric`, empty categories array, more than 20 categories, missing category `name`, `confidence_threshold` out of range |
