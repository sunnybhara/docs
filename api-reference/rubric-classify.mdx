...
<Note>
  Cost: **$0.003** per request (3 credits). Free keys are not charged credits.
</Note>
...
| `rubric.categories` | object[] | Yes | Array of category objects. Max 20 categories. Each must have a `name`. |
| `rubric.categories[].name` | string | Yes | Category name. Must be non-empty. |
| `rubric.categories[].description` | string | No | Optional description to guide the model. |
...
```json
{
  ...
  "success": true,
  "errors": [],
  "request_id": "req_nw_rub123abc456def789ghij01"
}
```
