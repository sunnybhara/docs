...
| `rubric.multi_label` | boolean | No | When `true`, return all categories above the confidence threshold. Default: `false`. |
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text` or `rubric`, empty categories, invalid category format, exceeds 20 categories |
