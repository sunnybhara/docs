...
Classify text against a user-defined rubric. Define your own categories with optional descriptions, enable multi-label classification, and set confidence thresholds.

<Note>
  Cost: **$0.003** per request (3 credits). **500 free requests/month** on this endpoint for all accounts.
</Note>
...
Each item in `rubric.categories` is an object:
...
The `description` field is optional but improves accuracy by providing context to the model.
...
```json
{
  ...
  "errors": [],
  "request_id": "req_nw_rub123abc456789012345678"
}
```
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text` or `rubric`, invalid categories, exceeds 20 category max |
