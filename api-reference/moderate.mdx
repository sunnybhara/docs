...
if (data.isSafe) {
  ...
  console.log('Cleaned:', data.cleanedText)
}
```
</CodeGroup>
...
```
  ...
  <Accordion title="High strictness catching mild content">
    With `high` strictness, NAWA flags borderline content that passes on `medium`:
```bash
curl -X POST https://api.trynawa.com/v1/moderate \
  -H "Authorization: Bearer nawa_live_sk_xxx" \
  -H "Content-Type: application/json" \
...
```
  ...
  <Accordion title="With platform context">
    Adding platform context helps the model calibrate its thresholds:
```bash
curl -X POST https://api.trynawa.com/v1/moderate \
  -H "Authorization: Bearer nawa_live_sk_xxx" \
  -H "Content-Type: application/json" \
