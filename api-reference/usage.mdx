...
When using `group_by=day`, the breakdown key changes to `by_day` and each item uses a `date` field (YYYY-MM-DD):
...
| `by_endpoint` or `by_day` | array | Breakdown grouped by the `group_by` parameter |

Each breakdown item contains `requests`, `cost`, and `cache_hits`.
