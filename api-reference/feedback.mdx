...
Submit reinforcement learning from human feedback (RLHF) to continuously improve NAWA's classification accuracy. When enough consistent feedback accumulates, patterns are automatically applied to improve future results.
...
| `corrected_intent` | string[] | No | Array of correct intent values if the model got it wrong. |
...
```json
{
  ...
  "errors": [],
  "request_id": "req_nw_fb_abc123def4"
}
```
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `classification_id`, invalid or missing `rating`, invalid `corrected_intent` (must be array of strings), invalid `corrected_sentiment` |
