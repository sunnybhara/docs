...
Submit reinforcement learning from human feedback (RLHF) to continuously improve NAWA's classification accuracy. Free for all tiers.
...
|-----------|------|----------|-------------|
| `classification_id` | string | Yes | The classification ID (`cls_nw_xxx`) from the original response. |
| `rating` | string | Yes | Your rating: `correct`, `incorrect`, `partial`. |
| `corrected_intent` | string[] | No | The correct intent values if the model got it wrong. |
...
curl -X POST https://api.trynawa.com/v1/feedback \
  ...
  -d '{
    ...
    "comment": "This is a suggestion, not a question. The user is recommending a topic."
  }'
```
