...
Submit reinforcement learning from human feedback (RLHF) to continuously improve NAWA's classification accuracy. Feedback is processed to identify patterns and correct model drift.
...
| `rating` | string | Yes | Accuracy rating: `correct`, `incorrect`, `partial`. |
...
curl -X POST https://api.trynawa.com/v1/feedback \
  ...
  -d '{
    "classification_id": "cls_nw_pon1yom9bkfe",
    "rating": "incorrect",
    "corrected_sentiment": "positive",
    "comment": "The comment was sarcastic praise, not negative"
  }'
```
...
const { data, error } = await nawa.feedback.submit({
  classificationId: 'cls_nw_pon1yom9bkfe',
  rating: 'incorrect',
