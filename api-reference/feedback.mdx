...
description: "Submit RLHF feedback on a classification result to improve NAWA's accuracy."
api: "POST https://api.trynawa.com/v1/feedback"
---

Submit feedback on a classification result to continuously improve NAWA's accuracy through reinforcement learning from human feedback (RLHF).
...
curl -X POST https://api.trynawa.com/v1/feedback \
  ...
  -d '{
    ...
    "comment": "This is clearly a complaint, not praise"
  }'
```
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `classification_id` or `rating`, or invalid `rating` value |
