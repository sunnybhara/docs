...
|-----------|------|----------|-------------|
| `classification_id` | string | Yes | The classification ID from the original response (`cls_nw_xxx` format). |
...
curl -X POST https://api.trynawa.com/v1/feedback \
  ...
  -d '{
    ...
    "comment": "This is a question about the next episode, not praise"
  }'
```
...
| `rating` | string | `correct`, `incorrect`, or `partial` |
