---
title: "Submit feedback"
sidebarTitle: "POST /v1/feedback"
description: "Submit feedback on classification results to improve accuracy over time."
api: "POST https://api.trynawa.com/v1/feedback"
---

Submit feedback on a classification result to continuously improve NAWA's accuracy. Rate classifications as correct, incorrect, or partial, and optionally provide corrected values.

<Note>
  This endpoint is **free** -- 0 credits, no cost.
</Note>

<Tip>
  Every feedback submission helps improve accuracy for all NAWA users. We especially value feedback on dialect detection edge cases.
</Tip>

## Request

### Body parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `classification_id` | string | Yes | The `id` from the original classification response (e.g., `cls_nw_a1b2c3d4e5f6`). |
| `rating` | string | Yes | Your assessment: `correct`, `incorrect`, or `partial`. |
| `corrected_intent` | string[] | No | Array of correct intent values if the classification was wrong (e.g., `["question", "suggestion"]`). |
| `corrected_sentiment` | string | No | Correct sentiment if wrong: `positive`, `negative`, `neutral`, `mixed`. |
| `comment` | string | No | Free-text explanation of why this correction is needed. |

### Example request

<CodeGroup>

```bash cURL
curl -X POST https://api.trynawa.com/v1/feedback \
  -H "Authorization: Bearer nawa_test_sk_xxx" \
  -H "Content-Type: application/json" \
  -d '{
    "classification_id": "cls_nw_a1b2c3d4e5f6",
    "rating": "incorrect",
    "corrected_intent": ["suggestion"],
    "corrected_sentiment": "neutral",
    "comment": "This is Lebanese Arabic, not Gulf. The user is making a suggestion, not a complaint."
  }'
```

```typescript TypeScript
import { Nawa } from '@nawalabs/sdk'

const nawa = new Nawa({ apiKey: process.env.NAWA_API_KEY })

const { data, error } = await nawa.feedback.submit({
  classificationId: 'cls_nw_a1b2c3d4e5f6',
  rating: 'incorrect',
  correctedIntent: ['suggestion'],
  correctedSentiment: 'neutral',
  comment: 'This is Lebanese Arabic, not Gulf.'
})
```

```python Python
from nawa import Nawa

nawa = Nawa(api_key="your_api_key")

result = nawa.feedback.submit(
    classification_id="cls_nw_a1b2c3d4e5f6",
    rating="incorrect",
    corrected_intent=["suggestion"],
    corrected_sentiment="neutral",
    comment="This is Lebanese Arabic, not Gulf."
)
```

</CodeGroup>

## Response

### Success response (200)

```json
{
  "success": true,
  "result": {
    "id": "fb_nw_xyz789",
    "object": "feedback",
    "classification_id": "cls_nw_a1b2c3d4e5f6",
    "rating": "incorrect",
    "acknowledged": true
  },
  "errors": [],
  "request_id": "req_fb_abc123"
}
```

### Result fields

| Field | Type | Description |
|-------|------|-------------|
| `id` | string | Unique feedback ID (`fb_` prefix) |
| `object` | string | Always `feedback` |
| `classification_id` | string | The classification this feedback references |
| `rating` | string | Your submitted rating |
| `acknowledged` | boolean | Always `true` on success |

### Error responses

| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `classification_id` or `rating`. Invalid `rating`, `corrected_sentiment`, or `corrected_intent` values. |
| 401 | `authentication_error` | Invalid or missing API key |
| 429 | `rate_limit_error` | Rate limit exceeded |
| 500 | `api_error` | Internal error |
