...
Classify a comment and generate an AI-powered reply in a single call. The reply matches the commenter's dialect and cultural context.
...
curl -X POST https://api.trynawa.com/v1/comments/reply \
  ...
  -d '{
    ...
    "context": {
      "platform": "youtube",
      "brand_voice": "Helpful and empathetic tech support"
    }
  }'
```
...
const { data, error } = await nawa.comments.reply({
  ...
  context: {
    platform: 'youtube',
    brandVoice: 'Helpful and empathetic tech support'
  }
})
```
...
```json
{
  ...
  "result": {
    ...
    "reply": {
      "text": "نقدر ملاحظتك ونأسف لتجربتك. نود مساعدتك لحل المشكلة. تواصل معنا عبر الخاص وبنساعدك بأسرع وقت.",
      "direction": "rtl",
      "tone": "professional"
    },
    "provider": "allam",
