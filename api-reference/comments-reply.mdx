...
```json
{
  ...
  "result": {
    ...
    "reply": {
      "text": "نعتذر عن تجربتك السلبية. نحرص على تحسين خدماتنا دائماً. هل يمكنك مشاركة تفاصيل أكثر حتى نتمكن من مساعدتك؟",
      "direction": "rtl",
      "tone": "professional"
    },
    "provider": "allam",
  ...
  "errors": [],
  "request_id": "req_nw_rep789xyz012abc"
}
```
...
| `classification.intent` | string[] | Detected intents from the input text |
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text`. Invalid tone or max_length. Text too long. |
