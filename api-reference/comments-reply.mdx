...
Classify a comment and generate a contextual reply in a single call. The reply matches the commenter's dialect and cultural context, with support for tone control and length limits.
...
console.log(result.reply.text)        // Generated reply
console.log(result.classification)    // Classification details
```
...
print(result.reply.text)        # Generated reply
print(result.classification)    # Classification details
```
...
```json
{
  ...
  "result": {
    ...
    "reply": {
      "text": "نأسف لتجربتك. نحرص دائماً على تقديم أفضل جودة. هل يمكنك مشاركة المزيد من التفاصيل حتى نساعدك؟",
      "direction": "rtl",
      "tone": "professional"
    },
    "provider": "allam",
...
| `classification.priority` | string | Suggested priority: `high`, `medium`, `low` |
