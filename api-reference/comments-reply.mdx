...
Classify a comment and generate a contextual, dialect-aware reply in a single API call. Send the comment text directly and receive both the classification and a generated reply.
...
print(result.reply.text)             # Generated reply in Arabic
...
console.log(data.reply.text)             // Generated reply in Arabic
...
```json
{
  ...
  "result": {
    ...
    "reply": {
      "text": "نعتذر عن تجربتك السلبية. نحرص على تحسين منتجاتنا باستمرار ونقدر ملاحظتك.",
      "direction": "rtl",
      "tone": "professional"
    },
    "provider": "allam",
...
| `classification.intent` | string[] | Detected intents of the input comment |
