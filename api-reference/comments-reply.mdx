...
Classify a comment and generate a contextual reply in a single call. The reply matches the commenter's dialect and supports tone control and max length configuration.

<Note>
  Cost: **$0.008** per request (8 credits). This is the most expensive endpoint because it combines classification and reply generation.
</Note>
...
```json
{
  ...
  "result": {
    ...
    "reply": {
      "text": "نقدر ملاحظتك ونأخذها بعين الاعتبار. نحرص دائماً على تقديم مراجعات صادقة وشاملة. إذا عندك اقتراحات محددة، نسعد بسماعها.",
      "direction": "rtl",
      "tone": "professional"
    },
    "provider": "allam",
...
| `classification.requires_response` | boolean | Whether NAWA recommends responding |
