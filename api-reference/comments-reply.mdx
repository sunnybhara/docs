---
title: "Generate reply"
sidebarTitle: "POST /v1/comments/reply"
description: "Classify a comment and generate a culturally-aware, dialect-matched reply in one call."
api: "POST https://api.trynawa.com/v1/comments/reply"
---

Classify and generate a reply to a comment in a single request. The reply matches the commenter's dialect and is contextually appropriate for the platform.

<Note>
  Cost: **$0.008** per request (8 credits). This endpoint classifies the comment and generates a reply in one call.
</Note>

## Request

### Headers

| Header | Required | Description |
|--------|----------|-------------|
| `Authorization` | Yes | `Bearer nawa_live_sk_xxx` or `Bearer nawa_test_sk_xxx` |
| `Content-Type` | Yes | `application/json` |

### Query parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `provider` | string | No | Override the AI provider: `claude`, `gemini`, `allam`. |

### Body parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `text` | string | Yes | The comment text to reply to. |
| `tone` | string | No | Reply tone: `friendly`, `professional`, `casual`, `formal`. Default: `professional`. |
| `max_length` | integer | No | Maximum reply length in characters (1-2000). Default: 500. |
| `context` | object | No | Additional context to improve reply quality. |
| `context.platform` | string | No | Source platform: `youtube`, `instagram`, `twitter`, `facebook`. |
| `context.brand_voice` | string | No | Description of your brand voice for tone matching. |

### Example request

<CodeGroup>

```bash cURL
curl -X POST https://api.trynawa.com/v1/comments/reply \
  -H "Authorization: Bearer nawa_live_sk_xxx" \
  -H "Content-Type: application/json" \
  -d '{
    "text": "متى الجزء الثاني؟",
    "tone": "friendly",
    "context": {
      "platform": "youtube",
      "brand_voice": "Tech review channel focused on smartphones"
    }
  }'
```

```typescript TypeScript
import { Nawa } from "@nawalabs/sdk";

const nawa = new Nawa({ apiKey: process.env.NAWA_API_KEY });

const result = await nawa.comments.reply({
  text: "متى الجزء الثاني؟",
  tone: "friendly",
  context: {
    platform: "youtube",
    brandVoice: "Tech review channel focused on smartphones",
  },
});

console.log(result.reply.text);
console.log(result.classification.intent);
```

```python Python
from nawa import Nawa

nawa = Nawa(api_key="nawa_live_sk_xxx")

result = nawa.comments.reply(
    text="متى الجزء الثاني؟",
    tone="friendly",
    context={
        "platform": "youtube",
        "brand_voice": "Tech review channel focused on smartphones",
    },
)

print(result.reply["text"])
print(result.classification["intent"])
```

</CodeGroup>

## Response

### Success response (200)

```json
{
  "success": true,
  "result": {
    "id": "rpl_a1b2c3d4e5f6",
    "object": "comment_reply",
    "classification": {
      "intent": ["question"],
      "sentiment": "neutral",
      "priority": "medium",
      "requires_response": true
    },
    "reply": {
      "text": "إن شاء الله الجزء الثاني قريب! تابعنا عشان ما يفوتك 🔔",
      "direction": "rtl",
      "tone": "friendly"
    },
    "provider": "allam",
    "model": "nagl-v1",
    "cost_usd": 0.008,
    "credits_used": 8
  },
  "errors": [],
  "request_id": "req_nw_rpl1a2b3c4d5e"
}
```

### Result fields

| Field | Type | Description |
|-------|------|-------------|
| `id` | string | Reply ID (`rpl_xxx` format) |
| `object` | string | Always `comment_reply` |
| `classification.intent` | string[] | Detected intents of the original comment |
| `classification.sentiment` | string | Detected sentiment |
| `classification.priority` | string | Priority level for response queue |
| `classification.requires_response` | boolean | Whether the comment warranted a reply |
| `reply.text` | string | The generated reply text |
| `reply.direction` | string | Text direction: `rtl` or `ltr` |
| `reply.tone` | string | The tone used for the reply |
| `provider` | string | AI provider used |
| `model` | string | Model version used |
| `cost_usd` | number | Cost in USD |
| `credits_used` | integer | Credits deducted |

### Error responses

| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text`. Invalid `tone` or `max_length`. |
| 401 | `authentication_error` | Invalid or missing API key |
| 402 | `insufficient_credits` | No credits remaining |
| 429 | `rate_limit_error` | Rate limit exceeded |
| 500 | `api_error` | Internal or provider error |
