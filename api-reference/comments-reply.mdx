...
description: "Generate a culturally-aware, dialect-matched reply to a comment."
api: "POST https://api.trynawa.com/v1/comments/{id}/reply"
---

Classify a comment and generate a contextual reply in a single call. Replies match the commenter's dialect and are appropriate for the detected platform. Supports tone control and max length configuration.
...
```bash cURL
curl -X POST "https://api.trynawa.com/v1/comments/cmt_abc123/reply?provider=allam" \
...
| `X-NAWA-Cache` | `HIT` if served from semantic cache |
