---
title: "Classify and generate reply"
sidebarTitle: "POST /v1/comments/reply"
description: "Classify a comment and generate a contextual, dialect-aware reply in a single call."
api: "POST https://api.trynawa.com/v1/comments/reply"
---

Classify a comment and generate a contextual reply in a single call. The reply matches the commenter's dialect and respects the tone you specify.

<Note>
  Cost: **8 credits ($0.008)** per request. Free tier keys (sandbox) are not charged credits.
</Note>

## Request

### Headers

| Header | Required | Description |
|--------|----------|-------------|
| `Authorization` | Yes | `Bearer nawa_live_sk_xxx` or `Bearer nawa_test_sk_xxx` |
| `Content-Type` | Yes | `application/json` |

### Query parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `provider` | string | No | Force a specific AI provider: `claude`, `gemini`, or `allam`. |

### Body parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `text` | string | Yes | The comment text to classify and reply to. Must be non-empty. |
| `tone` | string | No | Reply tone: `friendly`, `professional`, `casual`, or `formal`. Default: `professional`. |
| `max_length` | integer | No | Maximum reply length in characters (1 to 2000). Default: `500`. |
| `context` | object | No | Additional context with optional `platform` (string) and `brand_voice` (string) fields. |

### Example request

<CodeGroup>

```bash cURL
curl -X POST https://api.trynawa.com/v1/comments/reply \
  -H "Authorization: Bearer nawa_live_sk_xxx" \
  -H "Content-Type: application/json" \
  -d '{
    "text": "هذا المنتج سيء جداً ولا أنصح به",
    "tone": "professional",
    "max_length": 500,
    "context": {
      "platform": "youtube",
      "brand_voice": "helpful and empathetic"
    }
  }'
```

```typescript TypeScript
import { Nawa } from '@nawalabs/sdk'

const nawa = new Nawa({ apiKey: process.env.NAWA_API_KEY })

const result = await nawa.comments.reply({
  text: 'هذا المنتج سيء جداً ولا أنصح به',
  tone: 'professional',
  maxLength: 500,
  context: {
    platform: 'youtube',
    brandVoice: 'helpful and empathetic'
  }
})

console.log(result.reply.text)
console.log(result.classification.intent)  // ["complaint"]
```

```python Python
from nawa import Nawa

nawa = Nawa(api_key="nawa_live_sk_xxx")

result = nawa.comments.reply(
    text="هذا المنتج سيء جداً ولا أنصح به",
    tone="professional",
    max_length=500,
    context={
        "platform": "youtube",
        "brand_voice": "helpful and empathetic",
    },
)

print(result.reply.text)
print(result.classification.intent)  # ["complaint"]
```

</CodeGroup>

## Response

### Success response (200)

```json
{
  "result": {
    "id": "rpl_nw_a1b2c3d4e5f6",
    "object": "comment_reply",
    "classification": {
      "intent": ["complaint"],
      "sentiment": "negative",
      "priority": "high",
      "requires_response": true
    },
    "reply": {
      "text": "نعتذر عن تجربتك السيئة. نود مساعدتك - هل يمكنك مشاركة تفاصيل أكثر عن المشكلة؟",
      "direction": "rtl",
      "tone": "professional"
    },
    "provider": "allam",
    "model": "sdaia/allam-1-13b-instruct",
    "cost_usd": 0.008,
    "credits_used": 8
  },
  "success": true,
  "errors": [],
  "request_id": "req_nw_a1b2c3d4e5f67890abcdef12"
}
```

### Result fields

| Field | Type | Description |
|-------|------|-------------|
| `id` | string | Reply ID (`rpl_nw_xxx` format) |
| `object` | string | Always `"comment_reply"` |
| `classification.intent` | string[] | Detected intents of the original comment |
| `classification.sentiment` | string | Sentiment of the original comment |
| `classification.priority` | string | Priority level: `high`, `medium`, or `low` |
| `classification.requires_response` | boolean | Whether the comment requires a response |
| `reply.text` | string | The generated reply text |
| `reply.direction` | string | Text direction: `rtl` for Arabic replies, `ltr` for English |
| `reply.tone` | string | The tone used for the reply |
| `provider` | string | AI provider used |
| `model` | string | Specific model version |
| `cost_usd` | number | Cost in USD |
| `credits_used` | integer | Credits deducted (0 for sandbox keys) |

### Error responses

| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text`, invalid `tone`, `max_length` out of range |
| 401 | `authentication_error` | Invalid or missing API key |
| 402 | `permission_error` | No credits remaining |
| 429 | `rate_limit_error` | Rate limit exceeded |
| 500 | `api_error` | Internal or provider error |
