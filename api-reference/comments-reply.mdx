...
Classify a comment and generate a contextual reply in a single call. The reply matches the commenter's dialect and respects tone and length settings. Supports multi-provider routing via NAGL.

<Note>
  Cost: **$0.008** per request (8 credits). Not cached.
</Note>
...
```json
{
  "result": {
    ...
    "reply": {
      "text": "نعتذر عن تجربتك السيئة. نرجو التواصل معنا على الخاص لحل المشكلة.",
      "direction": "rtl",
      "tone": "professional"
    },
    "provider": "claude",
  ...
  "success": true,
  "errors": [],
  "request_id": "req_nw_rep789xyz012abcdef3456"
}
```
