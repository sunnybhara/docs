...
Translate text between English and Arabic with dialect awareness. Generates culturally appropriate translations in Gulf, Egyptian, Levantine, or MSA Arabic. Falls back to Claude if ALLaM is unavailable.
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text` or `target`. Invalid `source`, `target`, `dialect`, or `tone`. Text exceeds 5,000 characters. |
