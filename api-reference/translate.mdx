...
description: "Dialect-aware Arabic translation powered by ALLaM with Claude fallback."
api: "POST https://api.trynawa.com/v1/translate"
---

Translate text between English and Arabic with dialect awareness. Generates culturally appropriate translations in Gulf, Egyptian, Levantine, or MSA Arabic. Uses ALLaM as the primary provider with Claude Haiku as fallback.
...
<Warning>
  The `source` and `target` languages must be different. Sending `source: "ar"` with `target: "ar"` returns a 400 error.
</Warning>
...
| `fallback_used` | boolean | Whether the fallback provider (Claude) was used |
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text` or `target`. Same source and target language. Text too long. |
