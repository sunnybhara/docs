...
<Warning>
  The `source` and `target` languages must be different. Setting both to `ar` or both to `en` returns a `400 invalid_request_error`. To convert between Arabic dialects, use the `/v1/classify` endpoint to detect the dialect, then `/v1/translate` from `ar` to `en` and back with the desired target dialect.
</Warning>
