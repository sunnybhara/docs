...
<Warning>
  The `source` and `target` languages must be different. Setting `source` and `target` to the same value returns a 400 error.
</Warning>
...
| `model` | string | Model version used (e.g. `sdaia/allam-1-13b-instruct` or `claude-haiku-4-5-20251001`) |
...
```
  ...
  <Accordion title="Levantine Arabic to English">
...
```bash
    curl -X POST https://api.trynawa.com/v1/translate \
      -H "Authorization: Bearer nawa_live_sk_xxx" \
      -H "Content-Type: application/json" \
      -d '{"text": "كيف حالك اليوم؟", "source": "ar", "target": "en", "tone": "formal"}'
```
  </Accordion>
</AccordionGroup>
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text` or `target`. Invalid values. Text too long. `source` equals `target`. |
