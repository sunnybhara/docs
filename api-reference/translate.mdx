...
| `dialect` | string | No | Target dialect for Arabic output: `gulf`, `egyptian`, `levantine`, `msa`. Default: `msa`. Auto-detected when source is Arabic and dialect is not specified. |
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text` or `target`. Invalid values. Source equals target. Text too long. |
