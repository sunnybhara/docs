...
<Warning>
  Source and target languages must be different. Sending `source: "ar"` with `target: "ar"` returns a `400` error.
</Warning>
...
| `model` | string | Model identifier (e.g. `sdaia/allam-1-13b-instruct` or `claude-haiku-4-5-20251001`) |
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text` or `target`. Source equals target. Invalid dialect, tone, or platform. Text too long. |
