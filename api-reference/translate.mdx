...
| `source` | string | No | Source language: `ar`, `en`, or `auto`. Default: `auto` (detects automatically). |
| `dialect` | string | No | Target dialect for Arabic output: `gulf`, `egyptian`, `levantine`, `msa`. Default: `msa`. When source is Arabic, NAWA auto-detects the dialect if you omit this. |
...
curl -X POST https://api.trynawa.com/v1/translate \
  -H "Authorization: Bearer nawa_test_sk_xxx" \
  -H "Content-Type: application/json" \
  -d '{"text": "وش رايكم بالمحتوى الجديد؟", "target": "en", "tone": "formal"}'
```
  </Accordion>

  <Accordion title="English to Levantine Arabic (casual)">
    **Input:** "Thank you for watching!"
    **Output:** "يسلمو على المتابعة!"
```bash
curl -X POST https://api.trynawa.com/v1/translate \
  -H "Authorization: Bearer nawa_test_sk_xxx" \
  -H "Content-Type: application/json" \
  -d '{"text": "Thank you for watching!", "target": "ar", "dialect": "levantine", "tone": "casual"}'
```
  </Accordion>
</AccordionGroup>
