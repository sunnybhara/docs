...
| `source` | string | No | Source language: `ar`, `en`, `auto`. Default: `auto` (detects automatically). Must differ from `target`. |
...
```
  ...
  <Accordion title="English to Levantine Arabic (formal)">
    **Input:** "Thank you for your feedback, we will review it"
    **Output:** "شكرا على ملاحظاتكم، رح نراجعها"
```bash
    curl -X POST https://api.trynawa.com/v1/translate \
      -H "Authorization: Bearer nawa_test_sk_xxx" \
      -H "Content-Type: application/json" \
      -d '{"text": "Thank you for your feedback, we will review it", "source": "en", "target": "ar", "dialect": "levantine", "tone": "formal"}'
```
  </Accordion>
</AccordionGroup>
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing `text` or `target`. Invalid values. Text too long. `source` and `target` must differ. |
