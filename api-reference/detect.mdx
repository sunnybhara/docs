...
Detect language, dialect, script, and text direction from any text input. Uses local NAGL modules only -- no external AI call, so responses return in under 100ms. Results are cached for 5 minutes in Redis.
