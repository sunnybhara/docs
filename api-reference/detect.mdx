...
description: "Lightweight language and dialect detection. Returns in under 100ms with no external AI call."
...
const result = await nawa.detect({
  text: 'وش رايكم بالمحتوى الجديد؟'
})

console.log(result.data.language)    // "ar"
console.log(result.data.dialect)     // "gulf"
console.log(result.data.script)      // "arabic"
```
...
print(result.data.language)    # "ar"
print(result.data.dialect)     # "gulf"
print(result.data.script)      # "arabic"
```
...
| `cost_usd` | number | Cost in USD (always 0.002) |
| `credits_used` | integer | Credits deducted (always 2) |
...
<AccordionGroup>
  ...
  <Accordion title="Mixed script">
    **Input:** "awesome محتوى"
    **Result:** language: `mixed`, script: `mixed`, direction: `rtl`
...
| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Missing or empty `text`. Text too long. |
