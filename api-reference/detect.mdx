...
| `script` | string | Script type: `arabic`, `latin`, or `mixed`. A `latin` value for Arabic text indicates Arabizi (Arabic written in Latin characters). |
...
```
  ...
  <Accordion title="Arabizi (Latin-script Arabic)">
    **Input:** "7abibi el video 3ajabni ktir"
    **Result:** language: `ar`, dialect: `levantine`, script: `latin`, direction: `ltr`
```bash
curl -X POST https://api.trynawa.com/v1/detect \
  -H "Authorization: Bearer nawa_test_sk_xxx" \
  -H "Content-Type: application/json" \
  -d '{"text": "7abibi el video 3ajabni ktir"}'
```
  </Accordion>
