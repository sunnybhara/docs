---
title: "Analytics"
sidebarTitle: "GET /v1/analytics"
description: "Aggregated API analytics including request counts, costs, and cache performance."
api: "GET https://api.trynawa.com/v1/analytics"
---

Retrieve aggregated analytics for your API usage, including request counts, costs, cache performance, and breakdowns by endpoint, day, or provider. This endpoint is **free**.

## Request

### Query parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `from` | string | No | Start date (ISO 8601). |
| `to` | string | No | End date (ISO 8601). |
| `group_by` | string | No | Group breakdown by: `endpoint`, `day`, `provider`. Default: `endpoint`. |

### Example request

<CodeGroup>

```bash cURL
curl "https://api.trynawa.com/v1/analytics?from=2025-01-01T00:00:00Z&to=2025-01-31T23:59:59Z&group_by=endpoint" \
  -H "Authorization: Bearer nawa_test_sk_xxx"
```

```typescript TypeScript
import { Nawa } from '@nawalabs/sdk'

const nawa = new Nawa({ apiKey: process.env.NAWA_API_KEY })

const { data, error } = await nawa.analytics.get({
  from: '2025-01-01T00:00:00Z',
  to: '2025-01-31T23:59:59Z',
  groupBy: 'endpoint'
})
```

```python Python
from nawa import Nawa

nawa = Nawa(api_key="your_api_key")

result = nawa.analytics.get(
    from_date="2025-01-01T00:00:00Z",
    to_date="2025-01-31T23:59:59Z",
    group_by="endpoint"
)
```

</CodeGroup>

## Response

### Success response (200)

```json
{
  "success": true,
  "result": {
    "analytics": {
      "total_requests": 12450,
      "total_cost": 68.70,
      "cache_hits": 2835,
      "cache_hit_rate": 0.23,
      "avg_latency_ms": 820,
      "success_rate": 0.98,
      "error_count": 249,
      "endpoint_distribution": {
        "/v1/classify": 8500,
        "/v1/translate": 1200,
        "/v1/comments/reply": 1750,
        "/v1/detect": 500,
        "/v1/moderate": 500
      },
      "provider_distribution": {
        "allam": 9800,
        "claude": 2650
      },
      "period": {
        "from": "2025-01-01T00:00:00Z",
        "to": "2025-01-31T23:59:59Z"
      }
    },
    "by_endpoint": [
      {
        "endpoint": "/v1/classify",
        "requests": 8500,
        "cost": 51.0,
        "avg_latency_ms": 780
      },
      {
        "endpoint": "/v1/comments/reply",
        "requests": 1750,
        "cost": 14.0,
        "avg_latency_ms": 1200
      },
      {
        "endpoint": "/v1/translate",
        "requests": 1200,
        "cost": 6.0,
        "avg_latency_ms": 1100
      }
    ]
  },
  "errors": [],
  "request_id": "req_ana_abc123"
}
```

### Result fields

| Field | Type | Description |
|-------|------|-------------|
| `analytics.total_requests` | integer | Total API requests in the period |
| `analytics.total_cost` | number | Total credit cost in USD |
| `analytics.cache_hits` | integer | Number of requests served from cache |
| `analytics.cache_hit_rate` | number | Cache hit ratio (0--1) |
| `analytics.avg_latency_ms` | integer | Average response time in milliseconds |
| `analytics.success_rate` | number | Ratio of 2xx responses (0--1) |
| `analytics.error_count` | integer | Number of 4xx/5xx responses |
| `analytics.endpoint_distribution` | object | Request counts keyed by endpoint path |
| `analytics.provider_distribution` | object | Request counts keyed by AI provider |
| `analytics.period` | object | The queried date range |
| `by_endpoint` | array | Breakdown grouped by the `group_by` parameter |

<Note>
  When `group_by=day`, the breakdown key is `by_day` with each entry containing a `day` field (YYYY-MM-DD). When `group_by=provider`, the key is `by_provider`.
</Note>

### Error responses

| Status | Type | When |
|--------|------|------|
| 400 | `invalid_request_error` | Invalid `group_by` value |
| 401 | `authentication_error` | Invalid or missing API key |
| 429 | `rate_limit_error` | Rate limit exceeded |
| 500 | `api_error` | Internal error |
