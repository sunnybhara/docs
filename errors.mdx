  ...
  "errors": [
    {
      ...
      "param": "text",
      "doc_url": "https://docs.trynawa.com/errors#missing_required_param",
...
<AccordionGroup>
  ...
  <Accordion title="invalid_param_value">
    ...
    **Common triggers:**
    - `platform` not one of: `youtube`, `instagram`, `tiktok`, `twitter`, `facebook`
    ...
    **Fix:** Verify parameter values match the expected types and allowed values.
    ...
    ```json
    {
      ...
      "message": "Invalid value for 'platform'. Must be one of: youtube, instagram, tiktok, twitter, facebook.",
      "param": "platform",
...
<AccordionGroup>
  ...
  <Accordion title="invalid_api_key">
    ...
    **Fix:** Verify the key is correct and matches the environment (sandbox/production).
  ...
  <Accordion title="expired_api_key">
    **Cause:** The sandbox API key has exceeded its 14-day expiry.

    **Fix:** Create a new sandbox key from the dashboard. Production keys don't expire.

    ```json
    {
      "type": "authentication_error",
      "code": "expired_api_key",
      "message": "This sandbox API key has expired (14-day limit).",
      "suggested_action": "Create a new sandbox key from the dashboard."
    }
    ```
  </Accordion>
  ...
  <Accordion title="balance_exhausted">
    ...
    **Fix:** Purchase a credit pack from the dashboard. Enable auto top-up to prevent future interruptions.
    ...
    ```json
    {
      ...
      "suggested_action": "Buy a credit pack at developers.trynawa.com/dashboard/billing."
    }
    ```
  </Accordion>

  <Accordion title="grace_period_expired">
    **Cause:** The temporary grace period (min of 1 hour or $2 worth) after hitting $0 has ended.
    ...
    **Fix:** Purchase credits immediately. The grace period cannot be extended.
    ...
    ```json
    {
      ...
      "message": "Grace period has expired. Purchase credits to resume API access.",
      "suggested_action": "Buy a credit pack to restore access."
    }
    ```
  </Accordion>
</AccordionGroup>
  ...
  <Accordion title="rate_limit_exceeded">
    ...
    **Fix:** Wait until the `X-RateLimit-Reset` time, then retry. Consider upgrading your tier.
    ...
    ```json
    {
      ...
      "message": "Rate limit exceeded: 60 requests per minute for Trial tier.",
      "suggested_action": "Wait until the rate limit resets or upgrade your tier."
    }
    ```

    The `X-NAWA-RateLimit-Reason` response header provides additional context (e.g., `minute_limit`, `hour_limit`, `day_limit`).
  </Accordion>

  <Accordion title="sandbox_exhausted">
    **Cause:** You've used all 100 lifetime requests on your sandbox key.

    **Fix:** Upgrade to a production key with a credit pack.

    ```json
    {
      "type": "rate_limit_error",
      "code": "sandbox_exhausted",
      "message": "Sandbox key lifetime limit of 100 requests reached.",
      "suggested_action": "Upgrade to a production API key."
    }
    ```
  </Accordion>
</AccordionGroup>
