...
- Free and live tiers run independently -- free key requests never consume live credits
...
<CardGroup cols={1}>
  <Card title="Free tier" icon="flask">
    ...
    - Uses real AI models -- same accuracy as live
...
<Tip>
  Semantic cache hits (`X-NAWA-Cache: HIT`) cost **$0** -- even on billed endpoints. Classifying the same or semantically similar text is free after the first call.
</Tip>
...
- All billed endpoints return `402 insufficient_credits`
...
<Warning>
  There is no grace period. When credits hit zero, billed endpoints stop immediately. Enable monitoring on your `X-NAWA-Balance` header to avoid service interruption.
</Warning>
...
<AccordionGroup>
  <Accordion title="Do free key requests consume my live credits?">
    No. Free keys (`nawa_test_sk_`) have their own pool of 100 lifetime requests. Live credits are only consumed by live keys (`nawa_live_sk_`).
  </Accordion>
