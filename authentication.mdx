...
Keys are 32 hex characters after the prefix. Only the SHA-256 hash is stored -- you cannot retrieve a key after creation.
...
Your key's tier determines rate limits and feature access. Tiers auto-upgrade based on cumulative spend.

| Tier | Rate limit | Auto-upgrade at |
|------|-----------|----------------|
| Sandbox | 10/min | Free keys only |
| Trial | 60/min | $0 spend (first live key) |
| Growth | 120/min | $25 cumulative spend |
| Enterprise | 300/min | $50 cumulative spend |
| Enterprise Plus | 1,000/min | $200 cumulative spend |
...
<Steps>
  ...
  <Step title="Copy your key">
    ...
    <Warning>
      API keys are shown only once at creation. They are stored as SHA-256 hashes -- NAWA cannot retrieve your key later. If you lose it, revoke the old key and create a new one.
    </Warning>
  </Step>
</Steps>
...
Rotate live keys every **90 days**. To rotate without downtime:
...
// Never hardcode keys
const nawa = new Nawa({ apiKey: 'nawa_live_sk_abc123' }) // Don't do this
```
