...
|------|-------------|----------------|
| **Free** | 10 | Free keys (`nawa_test_sk_`), 100 lifetime cap |
| **Trial** | 60 | Live keys on initial credit purchase |
| **Growth** | 120 | Live keys (`nawa_live_sk_`) with active credits |
...
<Note>
  Free keys are rate-limited to 10 requests/minute and have a hard cap of 100 lifetime requests with a 14-day expiry. Trial keys allow 60 requests/minute during initial evaluation. Live keys with active credits start at the Growth tier (120/min). Enterprise tiers are available on request -- contact [sales@trynawa.com](mailto:sales@trynawa.com).
</Note>
