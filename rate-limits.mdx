...
NAWA enforces per-minute rate limits on each API key using a sliding window algorithm to ensure fair usage and platform stability.
...
| **Trial** | 60 | Live keys with minimal credits |
...
<Note>
  Free keys are rate-limited to 10 requests/minute and have a hard cap of 100 lifetime requests on paid endpoints. Live keys start at the Growth tier (120/min). Enterprise tiers are available on request -- contact [sales@trynawa.com](mailto:sales@trynawa.com).
</Note>
...
| `X-RateLimit-Reset` | When the current window resets (RFC 3339) | `2026-01-15T12:01:00Z` |
...
# On 429, read the Retry-After header and wait that many seconds
```
