...
NAWA enforces per-minute rate limits on each API key to ensure fair usage and platform stability. Limits are applied per API key using a sliding window.
...
| **Trial** | 60 | New accounts during the 7-day trial period |
...
<Note>
  Sandbox keys are rate-limited to 10 requests/minute and have a hard cap of 100 lifetime requests. Live keys start at Growth tier (120/min). Enterprise tiers are available on request.
</Note>
