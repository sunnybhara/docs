...
You can register up to **10 active webhook endpoints** per account using the API.
...
The response includes a `signing_secret` (format: `nawa_wh_xxx`). Store it securely since it is only shown once.
...
```bash
curl -X DELETE "https://api.trynawa.com/v1/webhooks?id=your-webhook-uuid" \
  -H "Authorization: Bearer nawa_live_sk_xxx"
```
...
Every webhook request includes an `X-NAWA-Signature` header containing the HMAC-SHA256 signature of the request body, and an `X-NAWA-Timestamp` header for replay protection.
...
After **10 consecutive failures**, the webhook endpoint is automatically **disabled** and you receive an email notification. Re-enable it from the dashboard after fixing the issue.

## Endpoint requirements

- URL must use HTTPS
- Your server must respond with a 2xx status code within 30 seconds
- Request body is JSON with `Content-Type: application/json`
