...
You can manage webhooks via the API:

- `POST /v1/webhooks` -- register a new endpoint (HTTPS only, up to 10 per account)
- `GET /v1/webhooks` -- list all registered endpoints
- `DELETE /v1/webhooks?id=<uuid>` -- deactivate an endpoint
...
The response includes a `signing_secret` (shown only once). Store it securely.
...
```json
{
  ...
  "data": {
    "request_id": "req_nw_def456ghi789",
...
Your webhook signing secret starts with `nawa_wh_` and is generated when you register a webhook endpoint. Store it securely -- it is shown only once.
