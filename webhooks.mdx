...
You can manage webhook endpoints via the API:

- **Create** -- `POST /v1/webhooks` with `url` (HTTPS only) and `events` array
- **List** -- `GET /v1/webhooks` to see all registered endpoints
- **Delete** -- `DELETE /v1/webhooks?id=UUID` to deactivate an endpoint

Maximum **10 active webhook endpoints** per account.

## Webhook signing secret

When you create a webhook endpoint, NAWA returns a signing secret prefixed with `nawa_wh_`. Store it securely -- it is shown only once at creation time.
